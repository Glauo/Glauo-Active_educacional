  ...
  return (
    <div className="student-shell">
      ...
      <main className="student-main">
        ...
        {tab === "financeiro" && <section className="student-panel"><div className="student-section-head"><div><span>Financeiro</span><h2>Pagamentos e mensalidades</h2></div></div>{faturas.length ? <table className="data-table"><thead><tr><th>Descrição</th><th>Vencimento</th><th>Valor</th><th>Status</th><th>Pagamento</th></tr></thead><tbody>{faturas.map((f, i) => { const boleto = boletoHref(f); const pix = pixHref(f); return <tr key={text(f.id) || i}><td>{text(f.descricao || f.categoria || "Mensalidade")}</td><td>{dateLabel(f.vencimento || f.data_vencimento)}</td><td>{money(valueOfInstallment(f))}</td><td><span className={`badge badge-${statusBadge(f.status || f.situacao)}`}><span className="badge-dot" />{text(f.status || "Pendente")}</span></td><td><div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>{pix && <a className="btn btn-secondary btn-sm" href={pix} target="_blank" rel="noreferrer">Pagar Pix</a>}{boleto && <a className="btn btn-secondary btn-sm" href={boleto} target="_blank" rel="noreferrer">Boleto</a>}{!pix && !boleto && "-"}</div></td></tr>; })}</tbody></table> : <Empty title="Sem faturas" desc="Nenhuma mensalidade encontrada para seu cadastro." />}</section>}
