...
export function ContratosClient({ alunos, contratos, isAdmin }: { alunos: Row[]; contratos: Row[]; isAdmin: boolean }) {
  ...
  function selectedStudent() { return alunos.find((student) => text(student.id) === form.aluno_id); }

  async function save() {
    if (!modal || busy) return;
    const student = selectedStudent();
    const payload: Row = modal.action === "inscricao" ? {
      action: "inscricao", aluno_id: text(student?.id), aluno: text(student?.nome || student?.name), aluno_login: text(student?.login || student?.usuario), curso: form.curso, data_inicio: form.data_inicio, valor_mensal: form.valor_mensal, observacoes: form.observacoes,
    } : {
  ...
  return <>
    <div className="page-header">
      ...
      <div className="page-actions"><button className="btn btn-primary" type="button" onClick={() => open("inscricao")}>Nova inscrição</button></div>
    </div>
