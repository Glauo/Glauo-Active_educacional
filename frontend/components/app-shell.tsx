...
import { navItemMessageKey } from "@/lib/nav-i18n";

type NavSectionKey = "principal" | "academic" | "management";

const navSections: { sectionKey: NavSectionKey; items: { href: string; icon: ReactNode }[] }[] = [
  {
      ...
      <aside className={`sidebar${mobileOpen ? " mobile-open" : ""}`}>
        <div className="sidebar-brand">
          ...
          <div className="brand-status">
            ...
            <span className="brand-status-text">{t("systemOperational")}</span>
          </div>
        </div>
        ...
        <nav className="sidebar-nav">
          {navSections.map((section) => (
            <div key={section.sectionKey}>
              <div className="nav-section-label">{tNav(`sections.${section.sectionKey}`)}</div>
              {section.items.filter((item) => canSeeNavItem(userRole, item.href, blockedRoutes) && (item.href !== "/usuarios/credenciais" || canManageAccess)).map((item) => {
                ...
                return (
                  ...
                  >
                    {item.icon}
                    <span className="nav-label">{tNav(`items.${navItemMessageKey(item.href)}`)}</span>
                  </a>
                );
              })}
            </div>
        ...
        <div className="sidebar-footer">
          <div className="sidebar-user-card">
            ...
            <div className="user-info">
              <div className="user-greeting">{t("greeting", { name: userName.split(" ")[0] })}</div>
      ...
      <div className="main-area">
        <header className="topbar">
          ...
          <div className="topbar-right">
            <LocaleSwitcher compact className="topbar-locale" />
            <button className="topbar-action-btn" title={t("notifications")}>
              <svg viewBox="0 0 20 20" fill="currentColor">
